...
void AC_LevelBuilderAI::AssignPrefabRatings()
{
    ...
    PrefabRatings["SM_prefab4_circle"] = {2, 2, 2, 2};
    PrefabRatings["SM_prefab5_circleObstacle"] = {3, 2, 2, 4};
    PrefabRatings["SM_prefab6_square"] = {2, 3, 3, 2};
    PrefabRatings["SM_prefab7_choosePath4"] = {2, 5, 4, 4};
    PrefabRatings["SM_prefab8_longWinding"] = {4, 6, 5, 5};
    PrefabRatings["SM_prefab9_triangle"] = {6, 1, 1, 0};
    PrefabRatings["SM_prefab10_triangleObstacle"] = {6, 2, 1, 1};
    PrefabRatings["SM_prefab11_cross"] = {7, 2, 0, 0};
    PrefabRatings["SM_prefab12_ledge"] = {0, 6, 4, 3};
    PrefabRatings["SM_prefab13_ledgeTall"] = {0, 7, 4, 4};
    ...
    PrefabRatings["SM_prefab22_mazeStandard"] = {8, 6, 8, 8};
...
FString AC_LevelBuilderAI::GenerateLevelGrammar()
{
    ...
    */
    ...
    UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerSkill);
    UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerScore);
    UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerMovement);
    UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerPreservation);
    ...
    while (PrefabPool.Num() > 0) //While there is a prefab still in the pool
    {
        ...
        UE_LOG(LogTemp, Log, TEXT("CURRENT PLAYER RATINGS"));
        UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerSkill);
        UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerScore);
        UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerMovement);
        UE_LOG(LogTemp, Log, TEXT("Player Skill Rating: %.2f"), PlayerPreservation);
        ...
        {
            UE_LOG(LogTemp, Log, TEXT("OVERBUDGET"));
            PlayerSkill += PrefabRatings[selectedPrefab][0];//Undo the math
            PlayerScore += PrefabRatings[selectedPrefab][1];
            PlayerMovement += PrefabRatings[selectedPrefab][2];
            PlayerPreservation += PrefabRatings[selectedPrefab][3];
            PrefabPool.RemoveAt(dieRoll); //removes the prefab that went overbudget from the pool

            //Then, if this removes the last prefab (meaning we've tried all the options), it will pop the Stack
            if (PrefabPool.Num() == 0) //If the prefab pool is somehow empty when the grammar is checked
            {
                if (GrammarStack.Num() > 0)
                {
                    UE_LOG(LogTemp, Log, TEXT("REMOVING OVERBUDGET FROM PREFAB POOL"));
                    UE_LOG(LogTemp, Log, TEXT("OVERBUDGET GRAMMAR STACK"));
                ...
                {
                    UE_LOG(LogTemp, Log, TEXT("CONDITION FAILED, EMPTYING PREFAB POOL"));
                    PrefabPool.Empty();
                }
        ...
        else if (PlayerSkill  <= budgetForgiveness &&  PlayerScore <= budgetForgiveness && PlayerMovement <= budgetForgiveness && PlayerPreservation <= budgetForgiveness)
        {
        ...
        else//If that prefab is insufficient to complete the cycle, continue the cycle
        {
            ...
            UE_LOG(LogTemp, Log, TEXT("Adding to Grammar Stack List"));
